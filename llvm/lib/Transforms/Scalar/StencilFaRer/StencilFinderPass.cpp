...
void buildBLASGEMMCall(Module &Mod, IRBuilder<> &IR,
                       const StencilFaRer::GEMM &Gemm) {

  const StencilFaRer::Matrix &MA = Gemm.getMatrixA();
  const StencilFaRer::Matrix &MB = Gemm.getMatrixB();
  const StencilFaRer::Matrix &MC = Gemm.getMatrixC();
...
void buildMMIntrinsicCall(IRBuilder<> &IR, const StencilFaRer::GEMM &Gemm) {
  ...
  MatrixBuilder<IRBuilder<>> MBuilder(IR);

  const unsigned BitsInAByte = 8;

  auto IsAColMajor = MA.getLayout() == StencilFaRer::ColMajor;
  auto const &AAlign = Align(AElType->getPrimitiveSizeInBits() / BitsInAByte);
  auto IsBColMajor = MB.getLayout() == StencilFaRer::ColMajor;
  ...
  Value *AVec =
      loadMatrixToFlatVector(MBuilder, APtr, *M, *K, *LDA, IsAColMajor, AAlign);
  Value *BVec =
      loadMatrixToFlatVector(MBuilder, BPtr, *K, *N, *LDB, IsBColMajor, BAlign);
  ...
  auto IsCColMajor = MC.getLayout() == StencilFaRer::ColMajor;
...
void buildEigenCall(Module &Mod, IRBuilder<> &IR, const StencilFaRer::GEMM &Gemm) {
  ...
  bool ARowMajor = MA.getLayout() == StencilFaRer::RowMajor;
  bool BRowMajor = MB.getLayout() == StencilFaRer::RowMajor;
  bool CRowMajor = MC.getLayout() == StencilFaRer::RowMajor;
